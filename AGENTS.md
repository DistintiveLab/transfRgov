...
- `skip_on_cran()` guards every mocked block in `test-pg_get.R` and every network-dependent block. `test-leitores.R` deliberately carries **no** `skip_on_cran()`: those tests touch no network, because a mocked response diverts `httr2::req_perform()` before any socket is opened, so skipping them on CRAN would only hide real coverage. The suite must stay hermetic (385 passing expectations, 0 failures) because `R CMD check` runs it on machines without internet access.
